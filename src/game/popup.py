...
class GamePopup:
    ...
    def draw(self, px_array):
        ...
        self.engine.screen.blit(self.bg_cache, (0, 0))

        new_px = pygame.PixelArray(self.engine.screen)
        
        try:
            self._draw_btn(new_px, self.btn_retry_rect, self.hover_retry)
            self._draw_btn(new_px, self.btn_menu_rect, self.hover_menu)
            ...
            for j in range(4):
                ...
                if aceito:
                    self.engine.renderer.draw_line(new_px, nx0, ny0, nx1, ny1, self.color_theme)
        finally:
            new_px.close()
